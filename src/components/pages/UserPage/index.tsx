...
import {
    ...
    useApproval_vendorMutation
} from "@/redux/apis/manageuser.api";
...
export default function UserManagementPage() {
    ...
    const {
        ...
        isLoading,
        error
    } = useGetAllUsersQuery(toQueryString(queryParams));
    const [updateUser] = useUpdateUserMutation();
    const [approvalVendor] = useApproval_vendorMutation();

    const handleApprovalVendor = async (user: User, status:string) => {
        const res = await approvalVendor({vendor_id: user.shop.id, status:status}) as unknown as {message: string}
        alert(`${res.message}`)
    }
            ...
            <div className="mx-auto p-6">
                ...
                <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
                    <div className="flex gap-2">
                        ...
                        <button
                            className={`px-4 py-2 rounded ${
                                activeTab === "vendor_request"
                                    ? "bg-blue-600 text-white"
                                    : "bg-gray-200 text-gray-800"
                            }`}
                            onClick={() => setActiveTab("vendor_request")}>
                            Duyệt cửa hàng
                        </button>
                    </div>
                        ...
                        <table className="w-full divide-y divide-gray-200 table-auto">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {users.length === 0 ? (
                                    ...
                                    users.map((user) => (
                                        <tr key={user.id}>
                                            <td className="px-6 py-4 text-sm text-gray-900 truncate max-w-[120px]">
                                                {user.username}
                                            </td>
