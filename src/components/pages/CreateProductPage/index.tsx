...
export default function CreateProductPage() {
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        ...
        try {
            ...
            const dataProductImage: ImageProduct[][] = data.products.map((product, index) => {
    ...
    const handleThumbnailChange = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        try {
            ...
            if (file) {
                ...
                if (urlFile && urlFile[0]) {
                    ...
                    newImages.splice(index, 0, urlFile[0]).filter(Boolean)
                    setPreviewImages(prev => [...newImages])
                } else {
    ...
    const handleImagesChange = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        try {
            const files = e.target.files;
            if (files) {
                const fileArr = Array.from(files).map((file) => file);
                const urlFile: string[] = await uploadFile(fileArr) as string[]
    ...
    console.log("check ddddddddd", previewImages)
    // JSX Rendering
    return (
        <ContainerLayout>
                                ...
                                <div className="w-full md:w-2/3 bg-white p-4 rounded">
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                {...register(`products.${index}.product_brand_id`, { required: "Thương hiệu là bắt buộc" })}
                                                className="mt-1 p-2 w-full border border-purple-300 rounded"
                                            >
                                            ...
                                            {errors.products?.[index]?.product_brand_id && (
                                                <p className="text-red-500 text-sm mt-1">{errors.products[index]?.product_brand_id?.message}</p>
                                            )}
                                        </div>
                                        <div>
